...
export function Submissions() {
  ...
  const handleVote = async (submissionId, voteType) => {
    try {
      ...
      // In a real app, you'd refresh the submission list or update the vote count
      
    } catch (e) {
      console.error("Error voting:", e);
      setMessage('');
      setError("Failed to submit vote. You may have already voted on this submission.");
    }
  };
    ...
    <div style={{ marginTop: '30px', borderTop: '2px solid #555', paddingTop: '20px' }}>
      ...
      {submissions.map(sub => (
        <div key={sub.id} style={{ border: '1px solid #ccc', padding: '10px', margin: '10px 0' }}>
          <h4>{sub.category} submission</h4>
          <p><b>Location:</b> ({Number(sub.lat).toFixed(5)}, {Number(sub.lon).toFixed(5)})</p>
          <p><b>Description:</b> {sub.description || 'N/A'}</p>
          <p><b>Suggested Building:</b> {sub.building_name_suggestion || 'N/A'}</p>
          <p><i>Submitted by: {sub.submitter_name}</i></p>
